    ...
    <div className="p-4 max-w-4xl mx-auto bg-black min-h-screen text-white">
      ...
      {userRole === 'admin' && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <KpiCard title="Flujo de Caja" value={`$${proyectos.reduce((acc, p) => acc + (p.presupuesto_total || 0), 0).toLocaleString()}`} icon={<Wallet size={20}/>} />
      ...
      {userRole === 'campo' && (
        <div className="bg-[#111] p-8 rounded-[2.5rem] border border-white/5">
          <h2 className="font-black text-xl mb-6 flex items-center gap-3"><HardHat className="text-[#facc15]" /> ACTIVIDADES</h2>
          <div className="space-y-4">
            {[
              "Cimentación", "Muros", "Eléctrica"
            ].map((task, i) => (
              <div key={i} className="flex items-center justify-between p-6 bg-white/5 rounded-2xl">
                ...
                <CheckCircle2 className="text-[#facc15]" />
              </div>
            ))}
          </div>
        </div>
...
// Función auxiliar para reporte financiero individual (restaurada y corregida)
